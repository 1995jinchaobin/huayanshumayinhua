              ...
              <el-col :span="12">
                <el-form label-width="100px">
                  <el-form-item label="来源:">
                    <label v-if="checkTime === true">{{
                      detailInfo.origin
                    }}</label>
                    <el-select v-model="detailInfo.origin" v-else>
                      <el-option
...
export default {
  ...
  methods: {
    ...
    add () {
      //判断
      if (!this.detailInfo.meter) return this.$message.error('请输入打印数量')
      if (this.detailInfo) {
        this.$put('/order/' + this.detailInfo.id, {
          'note2': this.detailInfo.note2 || '',
          'meter': this.detailInfo.meter,
          'origin': this.detailInfo.origin
        }).then((data) => {
