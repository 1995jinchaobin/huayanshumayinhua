<template>
  <div class="machineManageContent">
        ...
        <div class="checkDrawer">
          ...
          >
              ...
              <el-col :span="11" :offset="1">
                <el-form-item label="绑定的机型:">
                  ...
                  <el-select v-model="addParentId" placeholder="请选择">
                    <el-option
                      v-for="item in getDataParentList"
                      :key="item.id"
                      :label="item.name"
                      :value="item.id"
                    >
            ...
            <!-- <el-row>
              <el-col :span="20" :offset="1">
                <el-form-item label="普通产能比例:" prop="percentage">
                  <el-slider v-model.number="detailInfo.percentage" show-input>
                  </el-slider>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <el-col :span="11" :offset="1">
                <el-form-item label="备注:">
                  <el-input type="textarea" v-model="childNote"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
    ...
    <!-- <div>
      <el-dialog title="绑定打印员" :visible.sync="bindDrawer" width="80%">
        <el-row>
          <el-col :span="11" :offset="1">
  ...
  methods: {
    ...
    getDataParent () {
      ...
      this.$get('/machine', {
        ...
        this.getDataParentList = data.data.list
      })
    },
    search () {
      this.page.page = 1;
    ...
    check (row) {
      console.log(row)
      this.addParentId = row.machineId
      this.childName = row.name
      this.childNote = row.note
      this.updateDrawer = true
    ...
    closeCheckDrawer () {
      ...
      this.addParentId = this.childName = this.childNote = ''
    ...
    add () {
      ...
      //   return;
      // } else {
      if (!this.childName) return messageUtil.message.error("请输入机器名称")
      if (!this.addParentId) return messageUtil.message.error("请选择机型")
      const obj = {
        name: this.childName,
        note: this.childNote,
        machineId: this.addParentId
      }
      if (this.operateType == 'add') {
        console.log(obj)
        this.$post('/machine/child', obj).then((data) => {
          console.log(data)
        ...
        this.$put('/machine/child/' + this.detailInfo.id, obj).then((data) => {
          console.log(data)
          messageUtil.message.success(data.message)
          this.closeCheckDrawer();
          this.getData();
    ...
    HandlerDelete (machineInfo) {
      ...
      messageUtil.confirm('确认要删除该机器吗？', () => {
        this.$deletefn('/machine/child/' + machineInfo.id, {}).then((data) => {
          messageUtil.message.success(data.message)
          this.getData();
        })
      })
