  ...
  methods: {
    ...
    postAddFlawForm () {
      this.$refs.addFlawParamsRef.validate(async value => {
        ...
        if (this.title === '新增货品名称') {
          const res = await this.$post('/fabric/goods/spec', this.addFlawParams)
          messageUtil.message.success(res.message)
        } else {
          const res = await this.$put('/fabric/goods/spec/' + this.bianJiId, this.addFlawParams)
          messageUtil.message.success(res.message)
        }
        this.getFlawList()
        this.drawerAddClose()
    ...
    del (value) {
      messageUtil.confirm('确认要删除该货品名称吗？', async a => {
        const res = await this.$deletefn('/fabric/goods/spec/' + value.id, {})
        messageUtil.message.success(res.message)
        this.getFlawList()
      })
    }
...
.specification .searchArea {
  width: calc(100%);
  margin: 0px 24px 0px 0px;
}

.specification .searchForm {
  padding-top: 24px;
  padding-left: 32px;
}

.specification .dataListArea {
  margin: 0px 24px 24px 24px;
}

.specification .tableData {
  height: 100%;
}
.specification .addBottomBtn {
  display: flex;
  justify-content: center;
}
.specification .addBottomBtn .el-button {
  margin: auto 20px;
}
.specification .page {
  display: flex;
