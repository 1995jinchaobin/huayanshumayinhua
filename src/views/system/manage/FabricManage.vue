          ...
          <div class="searchForm">
            <el-form :inline="true" class="demo-form-inline">
              ...
              <el-form-item>
                ...
                <!-- <el-button plain @click="openUpdateDrawer">新增面料</el-button> -->
                <el-button plain @click="openUpdateDrawer"
                  >新增客户面料</el-button
                >
                <el-button plain @click="showJIngxiaoDraw"
                  >新增经销面料</el-button
                >
              </el-form-item>
            </el-form>
      ...
      >
        <div class="checkDrawer">
          ...
          >
            ...
            <el-form
              :model="mianliaoName"
              :rules="filterSearchRule"
              ref="filterSearchRef"
              label-width="100px"
              v-if="drawerTitle !== '编辑面料'"
            >
              <el-form-item label="货品名称:" prop="huopinName">
                <!-- <el-autocomplete
              ...
              ></el-autocomplete> -->
                <el-select
                  v-model="mianliaoName.huopinName"
                  placeholder="请选择货品名称"
                  clearable
                >
                  <div class="el-input" style="width: 90%; margin-left: 5%">
                    <input
                      ...
                      v-model="huopinSearchValue"
                      @input="dropDownSearch"
                    />
                  </div>
                  <el-option
                    v-for="item in huopinNameListFilter"
                    :key="item.id"
                    :value="item.name"
                  ></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="规格型号:" prop="guigeName">
                <el-select
                  v-model="mianliaoName.guigeName"
                  placeholder="请选择规格型号"
                  clearable
                >
                  <div class="el-input" style="width: 90%; margin-left: 5%">
                    <input
                      ...
                      placeholder="搜索"
                      class="el-input__inner"
                      v-model="guigeSearchValue"
                      @input="guigeDropDownSearch"
                    />
                  </div>
                  <el-option
                    v-for="item in guigeTypeListFilter"
                    :key="item.id"
                    :value="item.name"
                  ></el-option>
                </el-select>
            ...
            <el-row v-if="drawerTitle !== '编辑面料'">
              <el-col :span="12">
                <el-form-item label="面料名称:">
                  <el-input
                    :value="
                      mianliaoName.huopinName + '-' + mianliaoName.guigeName
                    "
                    disabled
                  ></el-input>
                </el-form-item>
      ...
      >
        <div class="jingxiao">
          ...
          >
            <el-form-item label="面料名称:">
              <el-input
                :value="mianliaoName.huopinName + '-' + mianliaoName.guigeName"
                disabled
              ></el-input>
            </el-form-item>
            <el-form-item label="警报阀值:" prop="threshold">
...
export default {
  data () {
    return {
      ...
      jingxiaoRules: {
        // name: [{ required: true, message: '请选择货品名称', trigger: 'change' }],
        percent: [{ required: true, message: '请输入缩水比例', trigger: 'blur' }],
        threshold: [{ required: true, message: '请输入警报阀值', trigger: 'blur' }]
      },
      guigeTypeList: [],
      ...
      huopinNameList: [],
      huopinSearchValue: '',
      huopinNameListFilter: [],
      filterSearchRule: {
        huopinName: [{ required: true, message: '请选择货品名称', trigger: 'change' }],
        guigeName: [{ required: true, message: '请选择规格型号', trigger: 'change' }],
      },
      mianliaoName: {
        huopinName: '',
        guigeName: '',
  ...
  methods: {
    ...
    //规格输入框搜索
    guigeDropDownSearch () {
      this.mianliaoName.guigeName = '';
      this.guigeTypeListFilter = this.guigeTypeList.filter(item => {
        return item.name.includes(this.guigeSearchValue)
      })
    },
    // 获取客户
    getUserList () {
    ...
    showDetail (row) {
      // console.log(row)
      // console.log(row.fkCustomerId)
      // console.log(this.userList)
      this.detailInfo = row;
      this.userList.forEach(item => {
        if (item.userDetail.fkUserId === row.fkCustomerId) {
          // console.log(item.userDetail.companyName)
          this.detailInfo.companyName = item.userDetail.companyName
        }
      })
    ...
    closeCheckDrawer () {
      ...
      if (this.drawerTitle !== '编辑面料') {
        this.$refs['filterSearchRef'].resetFields()
      }
    ...
    // 保存经销数据按钮
    addJingxiaoBtn () {
      this.$refs['filterSearchRef'].validate((valid1) => {
        if (!valid1) return
      ...
      this.$refs['fabricForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.operateType == 'add') {
            this.$refs['filterSearchRef'].validate((v) => {
              if (!v) return
              if (!this.customerName) return this.$message.error('请选择所属公司')
    ...
    async getGuigeList () {
      ...
      const res = await this.$get('/fabric/goods/spec', {
        ...obj
      })
      if (res.code !== 0) return messageUtil.message.error(res.message)
      this.guigeTypeList = res.data.list
      this.guigeTypeListFilter = res.data.list
    },
  },
  created () {
