<template>
  <div class="stockInfo">
    ...
    <el-table :data="tableData" border :header-cell-style="headFirst">
      ...
      <el-table-column prop="num4" align="center" label="生产流通量(米)">
      </el-table-column>
      <el-table-column align="center" prop="num3" label="成品仓(匹/米)">
      </el-table-column>
      ...
      >
        ...
        >
          ...
          <el-form-item label="半票仓匹数:" prop="rollNum">
            <el-input-number
              v-model="addStockParams.rollNum"
      ...
      >
        ...
        >
          <el-form-item label="上浆匹数:" prop="rollNum">
            <el-input-number
              v-model="addShangJiangParams.rollNum"
              controls-position="right"
              :min="0"
              :max="shangJiangRollNumMax"
              :precision="0"
            ></el-input-number>
          </el-form-item>
        ...
        <div class="bianjiKucunInfo">
          ...
          >
            ...
            <el-form-item label="半票仓(匹):" prop="rollNum">
              <el-input
                v-model="checkInfo.rollNum"
                type="text"
                placeholder="请输入半票仓(匹)"
              ></el-input>
              <!-- <label>{{detailInfo.id}}</label> -->
            </el-form-item>
            <el-form-item label="半票仓(米):" prop="num1">
              <el-input
                ...
                type="text"
                placeholder="请输入半票仓(米)"
              ></el-input>
              <!-- <label>{{detailInfo.id}}</label> -->
            </el-form-item>
            <el-form-item label="上浆仓(米):" prop="num2">
              <el-input
                v-model="checkInfo.num2"
                type="text"
                placeholder="请输入上浆仓(米)"
              ></el-input>
              <!-- <label>{{detailInfo.id}}</label> -->
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    dropDownSearch () {
      ...
      this.huopinNameListFilter = this.userList.filter(item => {
        return item.companyName.includes(this.huopinSearchValue)
      })
    },
    // 搜索
    searchList (value) {
    ...
    toAddMianliao () {
      if (this.addStockParams.fkCustomerId !== '') {
        ...
        const id = this.addStockUserName
        let key
    ...
    getUserList () {
      this.$get('/customer', {
        ...
        this.huopinNameListFilter = data.data.list
      })
    },
    // 公司名联系搜索框
    ...
    jingxiaoAdd () {
      const arr = this.userList.filter(item => {
        return item.companyName.includes('经销')
      })
      this.addStockParams.fkCustomerId = this.addStockUserName = arr[0].id
      this.getCategory()
    },
    // 选择客户名获取其ID
    getUserId1 (a) {
      this.addStockParams.fkCustomerId = a + ''
      this.getCategory()
    },
    // 面料品类
    getCategory () {
